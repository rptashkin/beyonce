...
make_palette_coachella <- function(path){
  ...
  # turn the unique rows into RGB hex values
  bey <- apply(k_means$centers, 1, function(x) rgb(x[1],x[2], x[3]))
  return(bey)
}

paths <- dir("coachella",full.names=TRUE)

coachella_colours <- lapply(paths, make_palette_coachella)
